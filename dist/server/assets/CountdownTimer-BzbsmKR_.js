...
function CountdownTimer({ compact = false }) {
  const offerDurationMs = 24 * 60 * 60 * 1e3;
  ...
  useEffect(() => {
    const getOfferEndsAt = () => {
      ...
      const endsAt2 = Date.now() + offerDurationMs;
      window.localStorage.setItem(storageKey, String(endsAt2));
      return endsAt2;
    };
    ...
    updateTimeLeft();
    const interval = window.setInterval(updateTimeLeft, 1e3);
    return () => window.clearInterval(interval);
  }, []);
  const totalSeconds = Math.floor(timeLeft / 1e3);
